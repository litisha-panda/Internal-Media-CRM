          ...
          <div style={{ padding:24 }}>
            {mode==="options" && (
              <>
                {/* Google */}
                <button
                  onClick={() => { setLoading(true); setTimeout(()=>{ setErr("Google OAuth requires Replit deployment. Use email login or Admin below."); setLoading(false); }, 600); }}
                  disabled={loading}
                ...
                {/* Zoho */}
                <button
                  onClick={() => { setLoading(true); setTimeout(()=>{ setErr("Zoho OAuth requires Replit deployment. Use email login or Admin below."); setLoading(false); }, 600); }}
                  disabled={loading}
...
export default function CROWarRoom() {
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflow:"auto",padding:20}}>
          ...
          {view==="ceo-risks" && isCEORole && (()=>{
            const highRisk=deals.filter(d=>d.quarter===filterQ&&d.outcome!=="Proposal Accepted"&&d.outcome!=="Not Interested").map(d=>({...d,idle:daysSince(d.lastContact)})).sort((a,b)=>(b.targetAmount-a.targetAmount)||(b.idle-a.idle));
            return(<div className="fin">
          ...
          {view==="md-accounts" && isMDRole && (()=>{
            const allD=deals.filter(d=>d.quarter===filterQ&&d.priority==="Top 5");
