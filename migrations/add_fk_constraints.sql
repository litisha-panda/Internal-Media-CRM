...
-- 1. Meetings with userId not in users:
--    SELECT m.id FROM meetings m LEFT JOIN users u ON m.user_id = u.id WHERE u.id IS NULL;

-- 2. Revenue entries with repId not in users:
--    SELECT r.id FROM revenue_entries r LEFT JOIN users u ON CAST(r.rep_id AS TEXT) = CAST(u.id AS TEXT) WHERE r.rep_id IS NOT NULL AND u.id IS NULL;

-- 3. Deals with repId not in users:
--    SELECT d.id FROM deals d LEFT JOIN users u ON CAST(d.rep_id AS TEXT) = CAST(u.id AS TEXT) WHERE d.rep_id IS NOT NULL AND u.id IS NULL;

-- 4. Tasks with assignedToUserId not in users:
--    SELECT t.id FROM tasks t LEFT JOIN users u ON t.assigned_to_user_id = u.id WHERE t.assigned_to_user_id IS NOT NULL AND u.id IS NULL;
