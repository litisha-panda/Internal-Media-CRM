...
- **T009 (Parts 12+13 — Navigation)**: RO Management card → `dealroreader.replit.app`. `getCRMDefaultView()` routes per role: ADMIN→admin-access, RH→warroom, NSH/CRO/Strategy→warroom, DIGI OPS→digi-deals, SALES REP→**target-submit** (targets first, per workflow simplification). Reset-All endpoint gated with `role !== "ADMIN"` check.
- **CRM Workflow Simplification**: All users bypass HomeScreen and land directly in CROApp on login (`setSection("crm")` for all roles). Sales Rep sidebar restructured (MY TARGETS → PLANNING → EXECUTION; War Room removed from rep view). Region Head sidebar simplified (MONITORING: War Room+Revenue Tracker; MY TEAM: Team's Plan, Team's Targets, My Targets, Approvals; MY WORK: My Plan, Revenue Log, Tasks, Internal Requests). Admin sidebar: ACCESS + PLATFORM (Target Import, Platform Config) + MONITOR (War Room, Revenue Tracker) + APPROVALS. "Platform launching soon" blocking screen replaced with soft inline banner only shown when no targets exist.
